...
class BaseModel
{
    ...
    {
        $model = new static;
        $model->sqlBuiler = "SELECT   * FROM $model->tableName  ORDER BY gia DESC LIMIT 3 ";
        $stmt = $model->conn->prepare($model->sqlBuiler);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_CLASS);
        return $result;
    }
    public static function allMoi()
    {
        $model = new static;
        $model->sqlBuiler = "SELECT * FROM $model->tableName ORDER BY id desc LIMIT 3";
        $stmt = $model->conn->prepare($model->sqlBuiler);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_CLASS);
        return $result;
